...
**What is MAID?**  
Moderated AI Development (MAID) is a methodology framework for teams using AI models (such as ChatGPT and Claude) to assist in software development. MAID provides roles, ceremonies, artefacts, quality gates, and prompt templates to ensure AI output is always reviewed and accepted by a qualified human before it enters the product. See the [manifesto](manifesto.md).

**Is MAID a replacement for Agile or Scrum?**  
No. MAID is a complementary framework that can be layered on top of Agile, Scrum, Kanban, or any other delivery methodology. MAID focuses specifically on the AI-assisted development workflow within a team's existing process.

**Do I need to use a specific AI model?**  
No. MAID is model-agnostic. The prompt templates provide versions for ChatGPT and Claude, but the principles apply to any AI model. Adapt the prompts to your chosen tool.

**Does MAID work for solo developers?**  
Yes. A solo developer can hold multiple MAID roles (Product Owner, Tech Lead, Moderator). The key discipline is maintaining the separation of concerns: plan separately, generate separately, review separately.
...
Quality gates exist to ensure that AI-generated artefacts meet a known standard of correctness before they enter the product. Waiving a gate means accepting unverified AI output — the exact risk MAID is designed to prevent. If a gate cannot be met, the correct response is to adjust the step scope or retry the prompt, not to skip the gate. See [quality-gates.md](quality-gates.md).
...
No. Moderation must be performed by a qualified human. Using one AI to evaluate another AI's output does not satisfy the MAID quality gate. The human moderator must understand and personally vouch for the accepted artefact.
...
MAID recommends Claude for the Development Team Agent role (code generation) and ChatGPT for Product Owner and Tech Lead Agent roles (planning and architecture). This is based on practical experience but is not prescriptive — use what works best for your team.

**Should prompts be stored in the repository?**  
Yes. Prompts are first-class artefacts in MAID and should be stored alongside the artefacts they produce. This enables traceability, reproducibility, and team learning.
...
**How do I share my experience with MAID?**  
